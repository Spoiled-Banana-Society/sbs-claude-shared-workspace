...
export async function GET(req: Request) {
  ...
  try {
    ...
    if (!isFirestoreConfigured()) {
      return json({ wheelSpins: 0, purchaseSpins: 0, freeDrafts: 0, jackpotEntries: 0, hofEntries: 0, draftPasses: 0, cardPurchaseCount: 0, cardFeeCreditCents: 0, cardFeeFrontGranted: false, firstPurchaseBonusGranted: false, firstPurchasePromoUnlocked: false, firstPurchaseVariant: 'new', hasSpunWheel: false, draftBlocked: false, supportBlocked: false });
    }
    ...
    return json({
      ...
      supportBlocked: data.supportBlocked === true,
    });
  } catch (err) {

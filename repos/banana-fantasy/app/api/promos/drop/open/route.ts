 ...
 *
 * Body: { userId, packId? }. Omit packId to open everything sealed (the
 * "open all fast" path). Opening is pure reveal — every prize was assigned when
 * the night locked at 8pm — so this cannot be raced into a better outcome.
 *
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // ⚠️ revealNightIdFor, NOT nightFor. nightFor rolls forward at 8pm, so
    // from the moment packs unlock this resolved to TOMORROW's empty night and
    // every open returned 'no-night' — "pack won't open" for everybody, at the
    // one hour the promo exists for (launch night, 2026-08-02).

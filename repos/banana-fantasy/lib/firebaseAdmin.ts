/**
 * Firebase Admin SDK singleton for server-side Firestore access.
 * 
 * Uses FIREBASE_SERVICE_ACCOUNT_JSON env var (base64-encoded or raw JSON)
 * to authenticate. Falls back to application default credentials.
 */

import { initializeApp, getApps, cert, type App } from 'firebase-admin/app';
import { getFirestore, type Firestore } from 'firebase-admin/firestore';
import { getDatabase, type Database as RtdbDatabase } from 'firebase-admin/database';

const STAGING_RTDB_URL = 'https://sbs-staging-env-default-rtdb.firebaseio.com';

let _app: App | null = null;
let _db: Firestore | null = null;
let _rtdb: RtdbDatabase | null = null;

// Staging service account (base64) — hardcoded fallback for Vercel runtime
// where .env.production server-side vars are not available.
const STAGING_SA_B64 = '';

function tryParseServiceAccount(source: string): { project_id?: string } | null {
  try {
    return JSON.parse(source);
  } catch {
    try {
      return JSON.parse(Buffer.from(source, 'base64').toString('utf8'));
    } catch {
      return null;
    }
  }
}

function getServiceAccount(): object | null {
  // 1) Try env var. If it's set but malformed (parse fails) OR parses but
  //    lacks project_id (truncated/corrupted), fall back to the hardcoded
  //    staging SA instead of returning null — null would make Admin SDK
  //    initialize with no credential and Firestore writes go to a phantom
  //    project where v2_error_events writes silently drop. This is the
  //    bug that hollowed out v2_error_events: the FIREBASE_SERVICE_ACCOUNT_JSON
  //    env on Vercel was set but unparseable, code returned null, app
  //    initialized with no project, every write disappeared.
  const raw = process.env.FIREBASE_SERVICE_ACCOUNT_JSON;
  if (raw && raw.trim()) {
    const parsed = tryParseServiceAccount(raw);
    if (parsed && parsed.project_id) {
      return parsed;
    }
    // Env var present but unusable — log once so it's visible in Vercel
    // runtime logs, then fall through to hardcoded fallback.
    console.warn(
      '[firebaseAdmin] FIREBASE_SERVICE_ACCOUNT_JSON env var is set but unparseable or missing project_id — using hardcoded staging fallback',
    );
  }

  // 2) Hardcoded staging fallback (works on Vercel where env vars sometimes
  //    end up corrupted across deploys / re-encrypting / re-imports).
  const fallback = tryParseServiceAccount(STAGING_SA_B64);
  if (fallback && fallback.project_id) {
    return fallback;
  }

  console.warn('[firebaseAdmin] Could not load a usable service account from env or fallback');
  return null;
}

export function getAdminApp(): App {
  if (_app) return _app;

  if (getApps().length) {
    _app = getApps()[0];
    return _app;
  }

  const sa = getServiceAccount();
  const databaseURL = process.env.NEXT_PUBLIC_DATABASE_URL || STAGING_RTDB_URL;
  if (sa) {
    _app = initializeApp({
      credential: cert(sa as Parameters<typeof cert>[0]),
      databaseURL,
    });
  } else {
    // Fall back to ADC (works in GCP environments)
    _app = initializeApp({ databaseURL });
  }
  return _app;
}

export function getAdminFirestore(): Firestore {
  if (_db) return _db;
  const app = getAdminApp();
  _db = getFirestore(app);
  return _db;
}

/**
 * Get the Realtime Database via Admin SDK. Bypasses RTDB security rules,
 * so use only from trusted server contexts (API routes, Cloud Functions).
 */
export function getAdminDatabase(): RtdbDatabase {
  if (_rtdb) return _rtdb;
  const app = getAdminApp();
  _rtdb = getDatabase(app);
  return _rtdb;
}

/**
 * Check if Firestore is configured (service account available).
 */
export function isFirestoreConfigured(): boolean {
  return !!process.env.FIREBASE_SERVICE_ACCOUNT_JSON || !!STAGING_SA_B64;
}

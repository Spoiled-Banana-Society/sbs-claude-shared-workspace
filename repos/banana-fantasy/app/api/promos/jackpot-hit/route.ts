...
export async function POST(req: Request) {
  ...
  try {
    ...
    // recordJackpotHit (resolveDraftPassType blocks free passes + per-draft
    // dedupe). Fully closing the "trust the client" gap — verify the caller
    // actually holds a paid token for this draft — is a separate, tested change.
    ...
    const promo = await recordJackpotHit(userId, draftId, passType);

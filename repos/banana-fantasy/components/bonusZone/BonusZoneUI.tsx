...
export const tierShort = (tier: 1 | 2 | null) => (tier === 1 ? 'BUY 1 GET 1' : tier === 2 ? 'BUY 2 GET 1' : 'CLOSED');
export const tierLabel = (tier: 1 | 2 | null) => (tier === 1 ? 'Buy 1 Get 1' : tier === 2 ? 'Buy 2 Get 1' : 'Zone closed');
  ...
  return (
    <div className="mb-2.5">
      ...
      <p className="text-[10.5px] leading-snug text-text-secondary">
        ...
        {view.tier === 2 && `Every 2 paid drafts you enter in the next ${plural(view.draftsLeftInTier, 'draft')} earn a free draft when they fill. Zone closes at draft ${t2}.`}
 ...
 */
export function BonusZoneLadder({ view, pending = 0, halves = 0 }: { view: BonusZoneViewLike; pending?: number; halves?: number }) {
  ...
  const bands: Array<{ from: number; to: number; l1: string; l2: string; on: boolean; cls: string }> = [
    ...
    { from: t2, to: 100, l1: 'NO BONUS', l2: `${t2 + 1} TO 100`, on: view.enabled && view.tier === null, cls: 'bg-white/[0.10]' },
  ];
  return (
    <div className="mt-2">
...
export interface BonusLockLike { tier: 1 | 2; label: string; credit: 1 | 0.5; eligible: boolean; reason: string }
...
export function BonusZoneEntryLine({ status, buyingSeat, firstPurchaseApplies }: { status: BonusZoneStatusLike | null; buyingSeat: boolean; firstPurchaseApplies: boolean }) {
  ...
  const earns = v.tier === 1 ? 'earns a free draft when it fills' : 'earns half a free draft when it fills, two fills make one';
  if (buyingSeat) {
  ...
  return (
    <p className="text-emerald-300 text-xs mt-0.5" data-testid="bonus-zone-entry-line">
      ...
      {(status.halvesThisWindow ?? 0) > 0 && v.tier === 2 && (
        <span className="block text-white/60">You have 1 of 2 banked this window. This fill completes the free draft.</span>
      )}
    </p>
  );
}
...
export function BonusLeaveWarning({ lock }: { lock: BonusLockLike | null | undefined }) {
  ...
  return (
    <p className="mb-6 -mt-3 rounded-lg border border-emerald-400/30 bg-emerald-400/[0.07] px-3 py-2 text-[13px] leading-snug text-emerald-200" data-testid="bonus-zone-leave-warning">
      Leaving forfeits this seat&apos;s Bonus Zone {lock.label}{lock.tier === 1 ? ' free draft' : ' half credit'}. Re-enter later and you lock whatever tier is live then.
    </p>
  );
}
...
export function BonusZoneModalContent({ data, rules }: { data: BonusZoneModalData | undefined; rules: string[] }) {
  const view: BonusZoneViewLike = {
    ...
    tier1Through: data?.tier1Through ?? 33,
    tier2Through: data?.tier2Through ?? 69,
  };
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ...
          { v: data?.halvesThisWindow ?? 0, l: 'HALF BANKED' },
        ].map((s) => (
            ...
            {data!.pending.map((p) => (
              <li key={p.draftId} className="flex items-center justify-between rounded-lg bg-white/[0.04] px-3 py-2 text-[12.5px]">
                ...
                <span className={p.eligible ? 'font-bold text-emerald-300' : 'text-white/40'}>
                  {p.eligible ? `${p.label} · ${p.tier === 1 ? 'free draft' : 'half'} pending` : 'not eligible'}
                </span>
              </li>
            ))}
          </ul>
            ...
            {data!.history.map((h) => (
              <li key={`${h.draftId}-${h.settledAtIso ?? ''}`} className="flex items-center justify-between rounded-lg bg-white/[0.04] px-3 py-2 text-[12.5px]">
                ...
                <span className={h.status === 'paid' ? 'font-bold text-emerald-300' : h.status === 'half' ? 'text-white/70' : 'text-amber-300'}>
                  {h.status === 'paid' ? '+1 free draft' : h.status === 'half' ? `${h.halvesAfter ?? 1} of 2` : 'retrying'}
                </span>
              </li>
            ))}
          </ul>

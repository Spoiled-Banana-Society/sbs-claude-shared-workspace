...
// Current contract = 1454 tokens (totalSupply). Refresh the real minted range.
const ids=[]; for(let i=1;i<=1460;i++) ids.push(i);
console.log(`refreshing ${ids.length} real tokens (1..1460)...`);
let ok=0,fail=0;
for(let i=0;i<ids.length;i+=4){
  ...
  await sleep(350);
  if((i+4)%200===0)process.stdout.write(`  ${i+4}/${ids.length} (ok=${ok} fail=${fail})\r`);
}
console.log(`\ndone: ok=${ok} fail=${fail}. OpenSea re-indexes async over the next few min.`);
process.exit(0);

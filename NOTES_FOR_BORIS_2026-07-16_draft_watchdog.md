...
- Root-cause polish: `submitPick` builds `PickNum` from Firestore `state/info` but `ProcessNewPick` validates against RTDB — when they diverge every pick is auto-rejected (that's what made 156 unrecoverable). Reading both from the same doc closes it.

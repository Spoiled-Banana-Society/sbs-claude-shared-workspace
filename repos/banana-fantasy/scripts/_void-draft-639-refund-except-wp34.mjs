// VOID 2026-fast-draft-639 (BBB #724) and refund every seat EXCEPT Wp34
// (0x3def2565eef4c0f949a2121415f8af126a7b4614) — Richard 8/15.
// Mirrors app/api/admin/drafts/manage/[slotId] DELETE exactly, minus wp34's
// refund. wp34's token stays in usedDraftTokens (pass consumed) but its league
// stamp is cleared so nothing references the deleted draft.
// Tracker (FilledLeaguesCount) is NOT touched: #725 already filled after this,
// so #724 becomes dead-but-counted (same treatment as BBB #133).
// Usage: node scripts/_void-draft-639-refund-except-wp34.mjs        (dry run)
//        CONFIRM=1 node scripts/_void-draft-639-refund-except-wp34.mjs
...
const ID = '2026-fast-draft-639';
const KEEP_BURNED = '0x3def2565eef4c0f949a2121415f8af126a7b4614'; // Wp34 — NO refund
const LEAGUE_LABEL = 'League #724';
...
if (draft.DisplayName !== 'BBB #724') { console.log('DisplayName mismatch:', draft.DisplayName, '— abort'); process.exit(1); }
...
for (const { OwnerId, TokenId } of members) {
  ...
  if (owner === KEEP_BURNED) {
    ...
    if (usedSnap.exists) batch.set(usedRef, { LeagueId: '', DraftType: '', LeagueDisplayName: '' }, { merge: true });
    await batch.commit();
    continue;
  }
  ...
  if (DRY) continue;
  const cleared = { ...src, LeagueId: '', DraftType: '', LeagueDisplayName: '' };
  const batch = db.batch();
  batch.set(db.doc(`draftTokens/${tokenId}`), { LeagueId: '', DraftType: '', LeagueDisplayName: '' }, { merge: true });
...
for (const { OwnerId, TokenId } of members) {
  ...
  console.log(`${owner.slice(0,10)} ${tokenId}: valid=${v.exists} used=${u.exists} stamp="${t.data()?.LeagueId}" type=${t.data()?.PassType} | user paid=${usr.draftPasses} free=${usr.freeDrafts}`);
}

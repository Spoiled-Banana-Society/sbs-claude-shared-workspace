...
## 2026-06-08 — FEATURE SPEC: sell a wheel-won JP/HOF pass while its draft is still filling

Richard wants: **when a user wins Jackpot or HOF from the wheel, mint them a real pass (like any other free draft) that they can LIST/SELL while the draft is still filling. Once it fills, it becomes their team and reverts to the normal "free team → listable after the season" rule.** The pass must be marked as a *filling JP/HOF league from the wheel*.

I'll build the **frontend/marketplace** side. The **Go draft engine** side is yours — flagging it because getting it wrong breaks live JP/HOF drafts, and it overlaps your pass work.

### Current reality (why this isn't a small tweak)
A JP/HOF wheel win today does **NOT** mint an NFT. In `app/api/wheel/spin/route.ts`: a JP/HOF win only bumps `jackpotEntries`/`hofEntries` and calls `joinQueue(userId, type)` → the user sits in `v2_queues/{jackpot|hof}` until **10 winners** stack up, then a draft is created. Only the *regular* draft-pass wheel prizes mint NFTs (`reserveTokensToWallet`, `pass_origin.origin='spin_reward'`, `registerMintedTokens(..., 'free')`). So **there's no asset to list** — that's the whole gap. Also: a pass normally only learns it's JP/HOF when its league *fills* (slot-machine reveal), and there's **no endpoint exposing "filling vs filled"** for an in-progress league.

### Backend tasks (you / Caleb)
1. **Mint a free pass on a JP/HOF wheel win** (can live in the Next.js spin route — I can write that — but it must be a pass the Go engine recognizes). Mark `pass_origin.origin='spin_reward'` + a level marker.
2. **Make the minted pass the thing that drafts.** Today the JP/HOF *queue* (keyed to wallet) is what forms the draft. For "sell before it fills" to mean anything, the draft slot has to follow the **NFT**, not the wallet — i.e. JP/HOF drafts should fill from these passes (membership keyed to tokenId/current owner) so a sale transfers the slot. This is the core change and it's in your engine.
3. **Set `_level` = `Jackpot`/`Hall of Fame` at mint** for these (it's *known* from the wheel, not a random reveal).
4. **Expose fill status to the frontend.** Cleanest: add `_leagueStatus: 'filling' | 'active' | 'completed'` to each token in `GET /owner/{wallet}/draftToken/all`. (Avoids a per-pass `/league/{id}` round-trip.)

### Data contract the frontend will read (please match these names, or tell me yours)
On each token from `draftToken/all` (→ `ApiDraftToken` → `MarketplaceTeam`):
- `_level`: `Jackpot` | `Hall of Fame` (set at mint for wheel JP/HOF) → drives `isJackpot`/`isHof`
- `passType`: `free`
- an origin marker that it's wheel-won (I can also resolve this from `pass_origin.origin='spin_reward'` via a token→origin lookup if you'd rather not add a field)
- `_leagueId`: set once it's in the filling league
- `_leagueStatus`: `filling` | `active` | `completed`  ← the key new field

### Frontend tasks (me) — ready to build against the contract above
- `SellTab` eligibility: today `canBeListed = !(passType==='free' && isDraftingOpen())`. Add an exception: **listable when `passType==='free' && (isJackpot||isHof) && wheelOrigin && leagueStatus==='filling'`.** After fill (`leagueStatus!=='filling'`) it's a normal drafted free team → existing "listable after the season" rule + messaging.
- Surface the new markers (`leagueStatus`, origin, level) through `app/api/marketplace/nfts/route.ts` + `MarketplaceTeam`.
- Listing card/label: "Jackpot/HOF pass — sellable until your draft fills."
- (Optional) I can add the JP/HOF mint to the spin route if you want minting to live there.

### Product decisions to lock (Richard + you)
- On a sale mid-fill, the **draft slot transfers to the buyer** (slot follows the NFT). Confirm.
- If a JP/HOF league never fills, what happens to a sold pass? (refund / stays listed / etc.)
- Does the buyer of a mid-fill JP/HOF pass keep the guaranteed JP/HOF, or only the slot?

I'm ready to build the frontend half as soon as the contract (esp. `_leagueStatus` + the wheel-origin marker) is confirmed. Ping me.

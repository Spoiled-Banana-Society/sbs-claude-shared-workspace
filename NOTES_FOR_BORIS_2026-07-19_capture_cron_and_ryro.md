...
## Addendum 2 (7/19 ~9pm): CryptoHX resolved — replacement NFT minted + linked (one stranded token to note)
Richard approved a replacement mint. **Token 2639** minted to CryptoHX's real wallet `0x09ce2805edf29d6bea03caeea72e808af8952a10` (tx 0x90360a95…) via the fulfill-failed-mints path (record `failed_mints/repair-cryptohx-hof12-873-v2`, wheel-origin/free, level hof) and fully linked as his HOF #12 team card — verified live (Team #2639 / Hall of Fame / League #12, owner feed resolves, OpenSea refreshed). Queue round-3 seat rebound 873→2639. Same repair pattern as Therec's 408→492.
⚠️ **Token 2638 is STRANDED**: my first mint went to a mistyped wallet tail (`0x09ce2805edcbb2…`, no account, no known key) — irreversible ERC-721, nobody can move it. Free-origin, $0 involved; its records are reverted to a plain pass and `pass_origin/2638` carries a STRANDED note; exclude from any token stats. My fault — wallet was hand-reconstructed instead of copied from source; process fixed (always copy full address from the card/queue doc, verify ownerOf before linking — did both for 2639).

...
# Freeze + winners bell + Packs off + seat everyone (last seat per league HELD on the Go side so nothing starts before 6). Wheel season switch is armed separately at 4:52
...
LOG=$HOME/Downloads/banana-race-5pm.log
{
  ...
  node scripts/_banana-race-seat.mjs --commit --hold-last
  echo "=== 5PM DONE $(date) ==="
} >> "$LOG" 2>&1

...
try {
  ...
  mark(`draft ${draftId}`);
  const url = `${BASE_URL}/draft-room?id=${encodeURIComponent(draftId)}&speed=${encodeURIComponent(STAGING_SPEED)}&staging=true&wallet=${encodeURIComponent(STAGING_WALLET)}&apiUrl=${encodeURIComponent(API_URL)}&debug=true`;

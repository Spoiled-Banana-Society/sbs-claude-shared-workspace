...
// Wheel odds (2026-07-20 JackHOF era): one 1-Draft wedge converted to JackHOF —
// a single draft carrying BOTH the Jackpot and HOF perks (0.1%). Jackpot (1%)
// and HOF (2%) untouched; the four remaining 1-Draft wedges absorb the 0.1%.
// Rarity strictly decreases: 1 > 2 > 5 > 10 > 20 > JackHOF.
// NOTE: changing any probability or segment id invalidates the ACTIVE VRF
// period's committed Merkle root — a config change must ship together with a
// period restart (close current → keeper opens the next with a fresh commit).
const DRAFT_ONE = 0.8915;   // across 4 wedges
...
const HOF = 0.02;
const JACKPOT = 0.01;
const JACKHOF = 0.001;      // 1 wedge — dual Jackpot+HOF draft

export const wheelSegments: WheelSegment[] = [
  { id: 'draft-1-a', label: '1 Draft', probability: DRAFT_ONE / 4, prizeType: 'draft_pass', prizeValue: 1, color: '#94a3b8' },
  { id: 'draft-5-a', label: '5 Drafts', probability: DRAFT_FIVE / 2, prizeType: 'draft_pass', prizeValue: 5, color: '#22c55e' },
  { id: 'draft-1-b', label: '1 Draft', probability: DRAFT_ONE / 4, prizeType: 'draft_pass', prizeValue: 1, color: '#94a3b8' },
  { id: 'jackpot', label: 'Jackpot', probability: JACKPOT, prizeType: 'custom', prizeValue: 'jackpot', color: '#ef4444' },
  { id: 'draft-1-c', label: '1 Draft', probability: DRAFT_ONE / 4, prizeType: 'draft_pass', prizeValue: 1, color: '#94a3b8' },
  ...
  { id: 'draft-1-d', label: '1 Draft', probability: DRAFT_ONE / 4, prizeType: 'draft_pass', prizeValue: 1, color: '#94a3b8' },
